...
// --- ¡NUEVO! Tabla para Cuentas de Referidos (Vendedores/Afiliados) ---
export const referrals = pgTable("referrals", {
  id: text("id").primaryKey().$defaultFn(() => createId()),
  name: varchar("name", { length: 256 }).notNull(),
  email: varchar("email", { length: 256 }).notNull().unique(),
  code: varchar("code", { length: 4 }).notNull().unique(), // Código de 4 dígitos para login y tracking (?r=CODE)
  commissionRate: decimal("commission_rate", { precision: 10, scale: 2 }).notNull().default("0.50"),
  isActive: boolean("is_active").default(true).notNull(),
  createdAt: timestamp("created_at").defaultNow().notNull(),
});

// --- Tabla para Links de Referidos (Campañas) ---
...
  id: text("id").primaryKey().$defaultFn(() => createId()),
  name: varchar("name", { length: 256 }).notNull(),
  slug: varchar("slug", { length: 256 }).notNull().unique(), // Campo slug único
...
  createdAt: timestamp("created_at").defaultNow().notNull(),
  // Campo para campañas de marketing (ej. Meta)
  referralLinkId: text("referral_link_id").references(() => referralLinks.id, { onDelete: "set null" }),
  // ✅ AÑADIDO: Campo para referidos con cuenta/panel
...
// ----------------------------------------------------------------
// TABLA PARA LA LISTA DE ESPERA (WAITLIST)
// ----------------------------------------------------------------
...
// --- ¡NUEVO! Relaciones para la tabla de Referidos con cuenta ---
export const referralsRelations = relations(referrals, ({ many }) => ({
  purchases: many(purchases),
}));

// --- Relaciones para la tabla de referidos de campañas ---
export const referralLinkRelations = relations(referralLinks, ({ many }) => ({
    purchases: many(purchases),
}));

// --- MODIFICADO: Añadir relación en purchases a la nueva tabla referrals ---
...
    }),
    tickets: many(tickets),
    // Relación para campañas (Meta)
    referralLink: one(referralLinks, {
        fields: [purchases.referralLinkId],
        references: [referralLinks.id],
    }),
    // ✅ AÑADIDO: Relación para referidos con cuenta
    referral: one(referrals, {
      fields: [purchases.referralId],

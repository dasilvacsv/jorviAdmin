...
// --- TIPOS ---
type WinnerTicket = { id: string; ticketNumber: string; purchase: { buyerName: string | null; buyerEmail: string; buyerPhone: string | null; } | null; };
type Purchase = { id: string; status: 'pending' | 'confirmed' | 'rejected'; buyerName: string | null; buyerEmail: string; ticketCount: number; amount: string; createdAt: Date; paymentReference: string | null; };
type RaffleTicket = { id: string; ticketNumber: string; status: 'available' | 'reserved' | 'sold'; purchaseId: string | null; };
type RaffleWithRelations = { id: string; name: string; description: string | null; price: string; currency: 'USD' | 'VES'; minimumTickets: number; status: "active" | "finished" | "cancelled" | "draft" | "postponed"; createdAt: Date; updatedAt: Date; limitDate: Date; winnerTicketId: string | null; winnerLotteryNumber: string | null; winnerProofUrl: string | null; winnerTicket?: WinnerTicket | null; images: { id: string; url: string; }[]; purchases: Purchase[]; tickets: RaffleTicket[]; };
type UnifiedTicketInfo = { id: string; ticketNumber: string; ticketStatus: 'available' | 'reserved' | 'sold'; buyerName: string | null; buyerEmail: string | null; purchaseStatus: 'pending' | 'confirmed' | 'rejected' | null; purchase: Purchase | null; };
type PurchaseWithTickets = Purchase & { ticketNumbers: string[]; };
type TopBuyer = { buyerName: string | null; buyerEmail: string; totalTickets: number; };
...
function ResendSubmitButton() { const { pending } = useFormStatus(); return (<Button type="submit" variant="ghost" size="sm" className="h-8 w-8 p-0 text-blue-600 hover:text-blue-800" disabled={pending} aria-label="Reenviar notificación">{pending ? <Loader2 className="h-4 w-4 animate-spin" /> : <Send className="h-4 w-4" />}</Button>); }
function ResendNotificationButton({ purchaseId }: { purchaseId: string }) { const [state, formAction] = useFormState(resendTicketsNotificationAction, { success: false, message: "" }); useEffect(() => { if (state.message) { alert(state.message); } }, [state]); return (<form action={formAction}><input type="hidden" name="purchaseId" value={purchaseId} /><ResendSubmitButton /></form>); }


// --- DEFINICIONES DE COLUMNAS COMO FUNCIONES ---

// 1. Convertimos la definición de columnas en una FUNCIÓN que acepta la lista completa de compras
const getUnifiedColumns = (allPurchases: Purchase[]): ColumnDef<UnifiedTicketInfo>[] => [
    ...
    { accessorKey: "buyerName", header: "Comprador", cell: ({ row }) => { const ticket = row.original; if (!ticket.buyerName) { return <span className="text-gray-400 italic">Disponible</span>; } return (<div className="text-left"><div className="font-medium text-sm">{ticket.buyerName}</div><div className="text-xs text-muted-foreground">{ticket.buyerEmail}</div></div>); } },
    { accessorKey: "purchaseStatus", header: "Estado", cell: ({ row }) => getStatusBadge(row.getValue("purchaseStatus")), filterFn: (row, id, value) => { if (!value.length) return true; const status = row.getValue(id); const ticketStatus = row.original.ticketStatus; if (value.includes('available')) { return ticketStatus === 'available' || value.includes(status); } return value.includes(status); }, },
    {
        id: "actions", header: "Acciones", cell: ({ row }) => {
            const ticket = row.original;
            if (!ticket.purchase) return null;

            const currentPurchase = ticket.purchase;

            // Lógica de detección de duplicados
            const exactDuplicate = allPurchases.find(p => p.paymentReference && currentPurchase.paymentReference && p.paymentReference === currentPurchase.paymentReference && p.id !== currentPurchase.id);
            const similarReferences = allPurchases.filter(p => p.paymentReference && currentPurchase.paymentReference && p.paymentReference.slice(-4) === currentPurchase.paymentReference.slice(-4) && p.id !== currentPurchase.id);
            const isDuplicate = !!exactDuplicate;
            const duplicateSaleId = exactDuplicate?.id;
            
            // Renderizamos el modal con las props
            return <PurchaseDetailsModal
                purchase={currentPurchase as any}
...
const getSalesColumns = (allPurchases: PurchaseWithTickets[]): ColumnDef<PurchaseWithTickets>[] => [
    { accessorKey: "buyerName", header: "Comprador", cell: ({ row }) => { const p = row.original; return (<div className="text-left"><div className="font-medium text-sm">{p.buyerName || 'N/A'}</div><div className="text-xs text-muted-foreground">{p.buyerEmail}</div></div>); }, },
    ...
    { accessorKey: "ticketCount", header: "Tickets", cell: ({ row }) => <div className="font-medium">{row.getValue("ticketCount")}</div> },
    {
        id: "actions", header: () => <div className="text-right">Acciones</div>, cell: ({ row }) => {
            const currentPurchase = row.original;
            ...
            return (
                <div className="flex items-center justify-end gap-1">
                    {currentPurchase.status === 'confirmed' && (<ResendNotificationButton purchaseId={currentPurchase.id} />)}
                    {/* Renderizamos el modal con las props */}
...
function UnifiedDataTable({ data, unifiedColumns }: { data: UnifiedTicketInfo[], unifiedColumns: ColumnDef<UnifiedTicketInfo>[] }) {
    const [columnFilters, setColumnFilters] = useState<ColumnFiltersState>([]);
    const [globalFilter, setGlobalFilter] = useState('');
    const table = useReactTable({ data, columns: unifiedColumns, getCoreRowModel: getCoreRowModel(), getPaginationRowModel: getPaginationRowModel(), onColumnFiltersChange: setColumnFilters, onGlobalFilterChange: setGlobalFilter, getFilteredRowModel: getFilteredRowModel(), state: { columnFilters, globalFilter }, globalFilterFn: (row, _, filterValue) => { const searchTerm = filterValue.toLowerCase(); return row.original.ticketNumber.includes(searchTerm) || row.original.buyerName?.toLowerCase().includes(searchTerm) || row.original.buyerEmail?.toLowerCase().includes(searchTerm); }, });
    return (
        <div>
            <div className="flex flex-col sm:flex-row items-center py-4 gap-4"><div className="relative w-full sm:flex-1"><Search className="absolute left-3 top-1/2 -translate-y-1/2 h-4 w-4 text-muted-foreground" /><Input placeholder="Buscar ticket, nombre, email..." value={globalFilter} onChange={(e) => setGlobalFilter(e.target.value)} className="pl-10" /></div><DropdownMenu><DropdownMenuTrigger asChild><Button variant="outline" className="w-full sm:w-auto sm:ml-auto">Estado <ChevronDown className="ml-2 h-4 w-4" /></Button></DropdownMenuTrigger><DropdownMenuContent align="end">{['available', 'confirmed', 'pending', 'rejected'].map((status) => (<DropdownMenuCheckboxItem key={status} className="capitalize" checked={table.getColumn("purchaseStatus")?.getFilterValue()?.includes(status) ?? false} onCheckedChange={(value) => { const currentFilter = table.getColumn("purchaseStatus")?.getFilterValue() as string[] | undefined || []; const newFilter = value ? [...currentFilter, status] : currentFilter.filter(s => s !== status); table.getColumn("purchaseStatus")?.setFilterValue(newFilter.length ? newFilter : undefined); }}>{status === 'confirmed' ? 'Confirmado' : status === 'pending' ? 'Pendiente' : status === 'rejected' ? 'Rechazado' : 'Disponible'}</DropdownMenuCheckboxItem>))}</DropdownMenuContent></DropdownMenu></div>
            <div>
                <Table><TableHeader>{table.getHeaderGroups().map(hg => (<TableRow key={hg.id} className="hidden md:table-row">{hg.headers.map(h => (<TableHead key={h.id} className={cn(h.id === 'ticketNumber' && 'w-[100px]', h.id === 'purchaseStatus' && 'w-[130px]', h.id === 'actions' && 'w-[120px] text-right',)}>{flexRender(h.column.columnDef.header, h.getContext())}</TableHead>))}</TableRow>))}</TableHeader>
                    <TableBody>{table.getRowModel().rows?.length ? (table.getRowModel().rows.map((row) => (<TableRow key={row.id} className="block rounded-lg border bg-card text-card-foreground shadow-sm mb-4 md:table-row md:border-b md:shadow-none md:mb-0">{row.getVisibleCells().map((cell) => (<TableCell key={cell.id} className="flex items-center justify-between gap-2 p-3 border-b last:border-b-0 md:table-cell md:p-4 md:border-b before:content-[attr(data-label)] before:font-semibold before:text-sm md:before:content-[]" data-label={flexRender(cell.column.columnDef.header, cell.getContext()) as string + ':'}><div className="text-right md:text-left w-full">{flexRender(cell.column.columnDef.cell, cell.getContext())}</div></TableCell>))}</TableRow>))) : (<TableRow><TableCell colSpan={unifiedColumns.length} className="h-24 text-center">No se encontraron resultados.</TableCell></TableRow>)}</TableBody></Table>
            </div>
            <div className="flex items-center justify-between space-x-2 py-4"><div className="flex-1 text-sm text-muted-foreground">{table.getFilteredRowModel().rows.length} de {data.length} ticket(s).</div><div className="flex gap-2"><Button variant="outline" size="sm" onClick={() => table.previousPage()} disabled={!table.getCanPreviousPage()}>Anterior</Button><Button variant="outline" size="sm" onClick={() => table.nextPage()} disabled={!table.getCanNextPage()}>Siguiente</Button></div></div>
        </div>
    );
}

function SalesDataTable({ data, salesColumns }: { data: PurchaseWithTickets[], salesColumns: ColumnDef<PurchaseWithTickets>[] }) {
    const [columnFilters, setColumnFilters] = useState<ColumnFiltersState>([]);
    const [globalFilter, setGlobalFilter] = useState('');
    const table = useReactTable({ data, columns: salesColumns, getCoreRowModel: getCoreRowModel(), getPaginationRowModel: getPaginationRowModel(), onColumnFiltersChange: setColumnFilters, onGlobalFilterChange: setGlobalFilter, getFilteredRowModel: getFilteredRowModel(), state: { columnFilters, globalFilter }, globalFilterFn: (row, _, filterValue) => { const searchTerm = filterValue.toLowerCase(); return row.original.buyerName?.toLowerCase().includes(searchTerm) || row.original.buyerEmail?.toLowerCase().includes(searchTerm); }, });
    return (
        <div>
            <div className="flex flex-col sm:flex-row items-center py-4 gap-4"><div className="relative w-full sm:flex-1"><Search className="absolute left-3 top-1/2 -translate-y-1/2 h-4 w-4 text-muted-foreground" /><Input placeholder="Buscar por nombre o email..." value={globalFilter} onChange={(e) => setGlobalFilter(e.target.value)} className="pl-10" /></div><DropdownMenu><DropdownMenuTrigger asChild><Button variant="outline" className="w-full sm:w-auto sm:ml-auto">Estado <ChevronDown className="ml-2 h-4 w-4" /></Button></DropdownMenuTrigger><DropdownMenuContent align="end">{['confirmed', 'pending', 'rejected'].map((status) => (<DropdownMenuCheckboxItem key={status} className="capitalize" checked={table.getColumn("status")?.getFilterValue()?.includes(status) ?? false} onCheckedChange={(value) => { const currentFilter = table.getColumn("status")?.getFilterValue() as string[] | undefined || []; const newFilter = value ? [...currentFilter, status] : currentFilter.filter(s => s !== status); table.getColumn("status")?.setFilterValue(newFilter.length ? newFilter : undefined); }}>{status === 'confirmed' ? 'Confirmado' : status === 'pending' ? 'Pendiente' : 'Rechazado'}</DropdownMenuCheckboxItem>))}</DropdownMenuContent></DropdownMenu></div>
            <div>
                <Table><TableHeader>{table.getHeaderGroups().map(hg => (<TableRow key={hg.id} className="hidden md:table-row">{hg.headers.map(h => (<TableHead key={h.id} className={cn(h.id === 'status' && 'w-[130px]', h.id === 'ticketCount' && 'w-[100px] text-center', h.id === 'actions' && 'w-[100px] text-right',)}>{flexRender(h.column.columnDef.header, h.getContext())}</TableHead>))}</TableRow>))}</TableHeader>
                    <TableBody>{table.getRowModel().rows?.length ? (table.getRowModel().rows.map((row) => (<TableRow key={row.id} className="block rounded-lg border bg-card text-card-foreground shadow-sm mb-4 md:table-row md:border-b md:shadow-none md:mb-0">{row.getVisibleCells().map((cell) => (<TableCell key={cell.id} className="flex items-center justify-between gap-2 p-3 border-b last:border-b-0 md:table-cell md:p-4 md:border-b before:content-[attr(data-label)] before:font-semibold before:text-sm md:before:content-[]" data-label={flexRender(cell.column.columnDef.header, cell.getContext()) as string + ':'}><div className={cn("w-full", cell.column.id === 'ticketCount' ? 'text-center' : 'text-right md:text-left')}>{flexRender(cell.column.columnDef.cell, cell.getContext())}</div></TableCell>))}</TableRow>))) : (<TableRow><TableCell colSpan={salesColumns.length} className="h-24 text-center">No se encontraron ventas.</TableCell></TableRow>)}</TableBody></Table>
            </div>
            <div className="flex items-center justify-between space-x-2 py-4"><div className="flex-1 text-sm text-muted-foreground">{table.getFilteredRowModel().rows.length} de {data.length} venta(s).</div><div className="flex gap-2"><Button variant="outline" size="sm" onClick={() => table.previousPage()} disabled={!table.getCanPreviousPage()}>Anterior</Button><Button variant="outline" size="sm" onClick={() => table.nextPage()} disabled={!table.getCanNextPage()}>Siguiente</Button></div></div>
        </div>
    );
}
...
export function RaffleDetailView({ initialRaffle, topBuyers }: { initialRaffle: RaffleWithRelations; topBuyers: TopBuyer[]; }) {
    ...
    // 2. Generamos las columnas DINÁMICAMENTE dentro del componente
    const unifiedColumns = useMemo(() => getUnifiedColumns(raffle.purchases), [raffle.purchases]);
    const salesColumns = useMemo(() => getSalesColumns(purchasesWithTicketsData), [purchasesWithTicketsData]);
    ...
    return (
        <div className="bg-slate-50 min-h-screen">
            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 md:py-12">
                <Link href="/dashboard/rifas" className="flex items-center gap-2 text-sm text-gray-600 hover:text-gray-900 transition-colors mb-6"><ArrowLeft className="h-4 w-4" /> Volver a todas las rifas</Link>
                        ...
                        <Card className="w-full">
                            <CardHeader>
                                <div className="flex flex-col sm:flex-row sm:justify-between sm:items-start gap-3">
                                    <div><CardTitle className="flex items-center gap-2 text-xl"><ListChecks className="h-6 w-6" /> Gestión de Participantes</CardTitle><CardDescription className="mt-1">Visualiza los datos por tickets, ventas o en el tablero interactivo.</CardDescription></div>
                                    <div className="flex flex-col sm:flex-row gap-2 w-full sm:w-auto">
                                        <Link href={`/rifas/${raffle.id}/bingo`} passHref><Button variant="outline" className="w-full sm:w-auto flex-shrink-0"><LayoutGrid className="mr-2 h-4 w-4" /> Ver Tablero (Bingo)</Button></Link>
                                        <Link href={`/rifas/${raffle.id}/analytics`} passHref><Button variant="outline" className="w-full sm:w-auto flex-shrink-0"><BarChart2 className="mr-2 h-4 w-4" /> Referidos</Button></Link>
                                        <Link href={`/rifas/${raffle.id}/ventas`} passHref><Button variant="outline" className="w-full sm:w-auto flex-shrink-0"><BarChart2 className="mr-2 h-4 w-4" /> Analizar Ventas</Button></Link>
                                    </div>
                                </div>
                            </CardHeader>
                            <CardContent>
                                <Tabs defaultValue="tickets" className="w-full">
                                    <TabsList className="grid w-full grid-cols-2"><TabsTrigger value="tickets">Por Ticket</TabsTrigger><TabsTrigger value="ventas">Por Venta</TabsTrigger></TabsList>
                                    {/* 3. Pasamos las columnas generadas a las tablas */}
                                    <TabsContent value="tickets" className="mt-4"><UnifiedDataTable data={unifiedTicketsData} unifiedColumns={unifiedColumns} /></TabsContent>
                                    <TabsContent value="ventas" className="mt-4"><SalesDataTable data={purchasesWithTicketsData} salesColumns={salesColumns} /></TabsContent>
                                </Tabs>
                            </CardContent>
                        </Card>
                    </div>

...
import { ArrowLeft, Users, DollarSign, Ticket, Crown, Loader2, Edit, Calendar as CalendarIcon, AlertCircle, AlertTriangle, Search, Star, ChevronDown, Phone, MessageSquare, BarChart2, Info, ListChecks, Award } from 'lucide-react';
...
// --- FUNCIONES Y COMPONENTES AUXILIARES (Sin cambios lógicos) ---
...
function TicketListModal({ticketNumbers,buyerName}:{ticketNumbers:string[],buyerName:string|null}){if(!ticketNumbers||ticketNumbers.length===0){return<span className="text-gray-400 italic">Sin asignar</span>;}return(<Dialog><DialogTrigger asChild><Button variant="outline" size="sm" className="h-8">Ver {ticketNumbers.length} ticket(s)</Button></DialogTrigger><DialogContent className="sm:max-w-md"><DialogHeader><DialogTitle>Tickets de {buyerName||'Comprador'}</DialogTitle><DialogDescription>Estos son todos los números asignados a esta venta.</DialogDescription></DialogHeader><ScrollArea className="h-72 w-full rounded-md border p-4"><div className="grid grid-cols-3 sm:grid-cols-4 gap-2 text-center">{ticketNumbers.map(number=>(<div key={number} className="font-mono p-2 bg-slate-100 rounded-md text-sm">{number}</div>))}</div></ScrollArea></DialogContent></Dialog>);}
...
const salesColumns: ColumnDef<PurchaseWithTickets>[] = [
    ...
    { accessorKey: "ticketNumbers", header: "Números", cell: ({ row }) => <TicketListModal ticketNumbers={row.original.ticketNumbers} buyerName={row.original.buyerName}/> },
                ...
                <main className="flex flex-col lg:flex-row gap-8 items-start">
                    <div className="flex-1 w-full space-y-8">
                        ...
                        <Card className="w-full">
                            <CardHeader>
                                {/* --- BOTÓN RESTAURADO Y HEADER MEJORADO --- */}
                                <div className="flex flex-col sm:flex-row sm:justify-between sm:items-start gap-3">
                                    <div>
                                        ...
                                        <CardDescription className="mt-1">Visualiza los datos por tickets o por ventas.</CardDescription>
                                    </div>
